<div role="status" id="toaster" x-data="toasterHub(@js($toasts), @js($config))" @class([
    ...
    'items-end rtl:items-start lg:-ml-72' => $position->is('right'),
 ])>
    <template x-for="toast in toasts" :key="toast.id">
        ...
        >
        <div class="flex items-cente px-6 py-3 rounded shadow-lgr"
        :class="toast.select({ error: 'bg-red-500', info: 'bg-gray-200', success: 'bg-green-600', warning: 'bg-orange-500' })">
            <template x-if="Object.keys(toast.select({ error: 'error' })).length > 0">
                <x-flux::icon.x-circle variant="solid"  />
            </template>
            <template x-if="Object.keys(toast.select({ info: 'info' })).length > 0">
                <x-flux::icon.information-circle variant="solid" />
            </template>
            <template x-if="Object.keys(toast.select({ success: 'success' })).length > 0">
                <x-flux::icon.check-circle variant="solid" />
            </template>
            <template x-if="Object.keys(toast.select({ warning: 'warning' })).length > 0">
                <x-flux::icon.exclamation-circle variant="solid"/>
            </template>

            <i x-text="toast.message"
               class="font-semibold text-sm w-full mt-0.5 ml-2"
